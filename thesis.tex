%% These files were modified in Lyx by:
%           Lus F. C. Fgueiredo, Fev., 2017.
%% Git repo for Lyx version are available at:
%%          https://github.com/lara-unb/DOCUMENTS/tree/master/templates
%% Latex version for sharelatex and overleaf v2 ported by:
%%          Vinícius Galvão Guimarães
%% 
%% Contributions of:
%%          Miguel Eduardo Gutierrez Paredes
%% LyX 2.2.3 created this file.  For more info, see http://www.lyx.org/.

%% IMPORTANTE!!!
%% Para fazer as junções com as folhas assinadas posteriormente,
%% recomendo usar o programa "pdftk", disponível para linux (apt) e windows.
%% Site: https://www.pdflabs.com/tools/pdftk-the-pdf-toolkit/
%% Ele preserva os link de citações, figuras, etc.
%% Comando: pdftk A=inA.pdf B=inB.pdf cat A1-12 B3 A14-end output out1.pdf

%\documentclass[11pt,oneside,portuguese]{book}
\documentclass[12pt,a4paper,oneside,english]{book}
\input{setup/document_def} %Este tem que ser o primeiro input!
\input{setup/packages}
\input{setup/other}
\input{setup/theorem_lemma}
\input{setup/languages}
\input{setup/listing_def}
\input{setup/glossaries_setup}
\usepackage{pdflscape}
\usepackage{adjustbox}
\usepackage{subfig}

\makeatother
\begin{document}
\pagenumbering{roman}

\input{setup/title_author_etc}
\input{parts/dedicated_to}

\runfrontend{}

\import{}{macros}
\import{parts/}{abstract}

\sumario

\listadefiguras

\listadetabelas


%-----------------------------------------------
% Use os simbolos, acronimos e notacoes com as funcoes a seguir
% To use this acronyms list, check the file glossaries-list/symbos.tex.
	\listadesimbolos

% To use this acronyms list, check the file glossaries-list/acronyms.tex.
	\listadeacronimos

% To use this acronyms list, check the file glossaries-list/notation.tex.
	\listadenotacoes
%-----------------------------------------------

\markboth{}{}

%CORPO PRINCIPAL
\mainmatter 
\setcounter{page}{1} \pagestyle{plain} 

\import{parts/}{capt_00}
\import{parts/}{capt_01}
\import{parts/}{capt_02}
\import{parts/}{capt_03}
\import{parts/}{capt_04}
\import{parts/}{capt_05}
%BIBLIOGRAPHY
\addcontentsline{toc}{chapter}{REFERÊNCIAS} 


% Do not use \bibliographystyle{IEEEtran} with abntex2cite package!!!!!
% Also, try not to use abntex2cite at all! just comment its package insetup/packages.tex
%\bibliographystyle{chicago}
% \bibliographystyle{IEEEtran}
%\bibliographystyle{acm}
\bibliography{bibliography}

%\part*{APPENDIX}
%\appendix
%\import{parts/}{appendix_resumo_estendido}
%\import{parts/}{appendix2}

\end{document}
